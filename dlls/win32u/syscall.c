...
#ifdef __SWITCH__
/* Statically linked into the runtime alongside ntdll's own unix funcs, so the
 * table is exposed under a unique name and registered via the static-unixlib
 * lookup in ntdll/unix/virtual.c (win32u.dll DllMain -> __wine_unix_call). */
const unixlib_entry_t wine_nx_win32u_unix_funcs[] =
{
    init,
};
#else
